...
bool operator==(Image const & im0, Image const & im1) {
	if(!(im0.width()==im1.width() && im0.height()==im1.height() && im0.size()==im1.size())) return false;
	else{
...
std::string to_string(Image const & im) {
	...
	for(int y=0;y<im.height();y++){
		for(int x=0;x<im.width();x++){
			res.append(std::to_string(im(x,y))+" ");
		}
		res.append("\n");
		}
  return res;
...
Image readPGM(std::string const & filename) {
  ...
  std::getline(pgm,line); //blank line from grayScaleValue
...
//Forward Declerations ---> see image.cc for implementation
